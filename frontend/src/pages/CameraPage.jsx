...
export default function CameraPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResult({
        status: "error",
        message: "Resim analiz edilirken bir hata oluştu.",
      });
    } finally {
  ...
  return (
    <div className="flex flex-col h-full bg-primary-dark p-4 overflow-y-auto">
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        {previewUrl ? (
          <div
            className="w-full relative rounded-2xl overflow-hidden shadow-2xl mb-6 bg-gray-800 flex items-center justify-center"
        ...
        {result && (
          <div
            className={`w-full mt-4 p-4 rounded-xl flex items-start gap-3 ${result.status === "error" ? "bg-red-900 border border-red-700" : "bg-gray-800 border border-primary-gold"}`}
          >
            <Info
              className={
                result.status === "error" ? "text-red-400" : "text-primary-gold"
              }
              size={24}
            />
            <div>
